...
| `src/jobagent/companion/` | The hosted tracker's local side (ADR 0010): `contract` (which board columns may go, held to the registry and the TypeScript contract by `tests/test_cloudflare_contract.py`), `sync` (the pure planner and its runner), `state` (what both sides last agreed), `client` (the only code that reaches the companion; refuses when the Access gate is off). |
