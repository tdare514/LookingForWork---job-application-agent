...
- 178 tests. `make check` runs ruff, strict mypy, pytest and the context check. CI runs the same on push.
...
3. Requirement extraction from posting text (#30), rule-based — descriptions now arrive, so there is finally something to extract from.
