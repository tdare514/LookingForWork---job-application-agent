...
- 484 tests pass, none skipped. Missing pursue, borderline or skip ranking tiers fail the suite (#76). `make check` runs ruff, strict mypy, pytest and the context check. CI runs the same on push.
...
- `--json` exists on `shortlist`, `digest` and `list` only; every other command prints a Rich table and composes with nothing. Global flags `--data-dir`, `--version` and `--verbose` (#121: each request on stderr, never a header, body or query string) exist; `--config` does not yet. Exit codes are documented in the README (#120).
